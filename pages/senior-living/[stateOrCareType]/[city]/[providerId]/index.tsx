...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const { page, templateAnnotations } = await getPage({
    pagePath: pagePath,
  });

  const slug =
    typeof context.params?.providerId == 'undefined'
      ? ''
      : isArray(context.params?.providerId)
      ? context.params?.providerId.join('/')
      : context.params?.providerId;

  const provider = await getProvider({
    slug:
      slug == 'provider' ? 'cambridge-village-of-apex-27502' : slug,
  });
...
export default function ProviderPage(
  ...
  const { page, templateAnnotations, pageParams, provider } = props;

  ParseMagnoliaPage({
    source: page,
    values: {
      stateOrCareType: pageParams.stateOrCareType
        ? pageParams.stateOrCareType
        : '',
